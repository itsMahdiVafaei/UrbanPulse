...
export default function TicketDetailsModal({ ticket, onClose }) {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            alert('متاسفانه در تولید فایل PDF مشکلی پیش آمد.');
        } finally {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-8 duration-500">
                ...
                <div ref={printRef} className="bg-white">
                    ...
                    {/* Header */}
                    <div className="px-10 py-8 bg-linear-to-br from-slate-50 to-white border-b border-slate-100 flex justify-between items-center" dir="rtl">
                        <div>
                        ...
                        {/* دکمه بستن - با ویژگی html2canvas-ignore در خروجی PDF نامرئی می‌شود */}
                        <button data-html2canvas-ignore onClick={onClose} className="p-3 bg-slate-100 hover:bg-slate-200 rounded-2xl transition-all cursor-pointer">
                    ...
                    <div className="p-10 overflow-y-auto max-h-[65vh] space-y-6 text-right" dir="rtl">
                        ...
                        {/* علت رد درخواست (فقط برای وضعیت REJECTED) */}
                        ...
                        {/* وضعیت ارجاع (برای وضعیت در حال انجام) */}
                ...
                {/* Footer Actions */}
                <div className="px-10 py-6 bg-slate-50 border-t flex flex-col sm:flex-row justify-between gap-4" dir="rtl">
                    <div className="flex gap-4 w-full">
                        {/* دکمه دانلود PDF */}
                        <button
                            onClick={handleDownloadPdf}
                            disabled={isDownloading}
                            className="flex-[2] py-3 text-sm font-black text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
                        >
                            <Download size={18} />
                            {isDownloading ? 'در حال آماده‌سازی...' : 'دانلود رسید PDF'}
                        </button>

                        {/* دکمه بستن */}
                        <button
                            onClick={onClose}
                            className="flex-1 py-3 bg-slate-200 hover:bg-slate-300 text-slate-600 rounded-xl font-bold text-xs cursor-pointer transition-all"
                        >
                            بستن
                        </button>
                    </div>
...
function InfoItem({ icon, label, value, isCode }) {
    return (
        <div className="flex items-center gap-3 p-2">
            <div className="text-blue-500 bg-white p-2 rounded-xl shadow-sm border border-blue-50">{icon}</div>
            <div className="flex flex-col">
